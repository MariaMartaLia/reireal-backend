...
public void aprovarPagamento(){
    if(status != StatusPagamento.AGUARDANDO_PAGAMENTO){
        throw new IllegalStateException("Pagamento só pode ser aprovado se estiver AGUARDANDO_PAGAMENTO.");
    }
...
public void recusarPagamento(){
    if(status != StatusPagamento.AGUARDANDO_PAGAMENTO){
        throw new IllegalStateException("O pagamento só pode ser recusado se estiver AGUARDANDO_PAGAMENTO.");
    }
    status = StatusPagamento.PAGAMENTO_RECUSADO;
}
public void creditarPagamento(){
    if(status != StatusPagamento.PAGAMENTO_APROVADO){
        throw new IllegalStateException("O pagamento só pode ser creditado se estiver PAGAMENTO_APROVADO.");
    }
    status = StatusPagamento.PAGAMENTO_CREDITADO;
}
public void cancelarPagamento(){
    if(status != StatusPagamento.AGUARDANDO_PAGAMENTO){
        throw new IllegalStateException("O pagamento só pode ser cancelado se estiver AGUARDANDO_PAGAMENTO. ");
    }
...
private void validarValor(BigDecimal valor){
    if(valor == null ||valor.compareTo(BigDecimal.ZERO) <= 0){
        throw new IllegalArgumentException("O valor do pagamento não pode ser nulo e deve ser maior que zero.");
    }
}
